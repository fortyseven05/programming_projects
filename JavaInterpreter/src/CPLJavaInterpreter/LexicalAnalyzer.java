...
public class LexicalAnalyzer
{
    ...
    private TokenType getTokenType(String lexeme, int lineNumber, int columnNumber) throws LexicalException
    {
        ...
        if (Character.isLetter(lexeme.charAt(0)))
        {
            ...
            else if (lexeme.length() > 1)
            {
                if (lexeme.equals("for"))
                    tokType = TokenType.FOR_KW_TOK;
                else if (lexeme.equals("function"))
                    tokType = TokenType.FUNC_KW_TOK;
                else if (lexeme.equals("print"))
                    tokType = TokenType.PRINT_KW_TOK;
                else if (lexeme.equals("end"))
                    tokType = TokenType.END_KW_TOK;
                else if (lexeme.equals("if"))
                    tokType = TokenType.IF_KW_TOK;
                else if (lexeme.equals("else"))
                    tokType = TokenType.ELSE_KW_TOK;
                else if (lexeme.equals("while"))
                    tokType = TokenType.WHILE_KW_TOK;
                else
                    throw new LexicalException ("invalid lexeme at row number " +
                        (lineNumber + 1) + " and column " + (columnNumber + 1) + "" + lexeme);
        ...
        else if (Character.isDigit (lexeme.charAt(0)))
        {
            ...
            tokType = TokenType.OPEN_PAREN_TOK;
        else if (lexeme.equals(")"))
            tokType = TokenType.CLOSE_PAREN_TOK;
        else if (lexeme.equals(":"))
            tokType = TokenType.ITER_TOK;
        else if (lexeme.equals("=="))
